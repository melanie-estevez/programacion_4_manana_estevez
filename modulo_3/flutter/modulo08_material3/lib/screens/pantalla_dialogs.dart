...
class PantallaDialogs extends StatelessWidget {
  ...
  void _funcionnuevaSnack(BuildContext context, {bool esError = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          esError
              ? 'Error: no sirve el snackbar nuevo'
              : 'Conexión establecida correctamente al nuevo snackbar',
        ),
        backgroundColor:
            esError ? Theme.of(context).colorScheme.error : null,
        action: SnackBarAction(
  ...
  Future<void> _mostraralertanueva(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        icon: const Icon(
        ...
        title: const Text('Alerta de Servidor Eliminado'),
        content: const Text(
          'Dessea Eliminar?'
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                controller: ctrlIp,
                decoration:
                    const InputDecoration(labelText: 'Dirección IP'),
                validator: (v) {
                  if (v == null || v.isEmpty) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          OutlinedButton.icon(
            onPressed: () => _funcionnuevaSnack(context),
            icon: const Icon(Icons.rectangle_rounded),
            label: const Text('SnackBar nuevo'),
          ),
          ...
          OutlinedButton.icon(
            ...
            label:
                const Text('Eliminar servidor (confirmación)'),
          ),
          ...
          FilledButton.tonal(
            ...
            child:
                const Text('Agregar servidor (formulario)'),
          ),

          const SizedBox(height: 8),

          OutlinedButton(
            onPressed: () => _mostraralertanueva(context),
            child: const Text('Alert nuevo'),
          ),
        ],
      ),
    );
